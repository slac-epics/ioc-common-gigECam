...
# ----- Camera UI -----
from manta046B_ui import Ui_GigEImageViewer
...
from PyQt4.QtGui import QWidget, QImage, QPainter, QColor
...
class Camera():
    ...
    def __init__(self, name):
        logging.debug("Creating camera %s", name)
        self.name = name
...
class GraphicUserInterface(QMainWindow, Ui_GigEImageViewer):
    ...
    def go(self, val):
        ...
        logging.debug("cam_pv = %s", cam_pv)
        logging.debug("imagepv = %s", self.image_pv)
    ...
    def UpdateImage(self, exception=None):
        try:
          if exception is None:
            ...
            logging.debug("%-30s %s", self.name, exception)
        except Exception, e:
    ...
    def updateImageCounter(self, exception=None):
        ...
        try:
          if exception is None:
            ...
            data = pv.value
            #logging.debug("ImageCounter = %d", data)
    ...
    def updateCross2Color(self, exception=None):
        ...
        try:
          if exception is None:
            ...
            data = pv.value
            self.cbCross2Color.setCurrentIndex(data)
          else:
            logging.debug("%-30s %s", self.name, exception)
            self.disp_val(self.lGain, -1, None)
        except Exception, e:
